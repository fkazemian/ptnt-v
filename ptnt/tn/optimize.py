...
#
# Automatic‑differentiation back‑ends + vectorization + training loop
...
class JaxHandler:
    ...
    def setup_fn(self, fn):
        jax = get_jax()
        if self.jit_fn:
            self._backend_fn = jax.jit(fn)
            self._value_and_grad = jax.jit(jax.value_and_grad(fn))
        else:
            self._backend_fn = fn
...
class TorchHandler:
    ...
    def value_and_grad(self, arrays):
        ...
        torch_grads = torch.autograd.grad(
            result, variables, allow_unused=True
        )
        grads = [
            # unused variables return as None
            np.zeros_like(arrays[i])
            if g is None
            else to_numpy(g).conj()
...
class TNOptimizer:
    ...
    Additional PTNT-specific arguments
    ----------------------------------
    causality_fn : callable
        Mapping from random Pauli key tuples to operator arrays used for
        causality regularization.
    training_data : array-like
        Counts/probabilities for training.
    training_sequences : array-like
        Operator sequences for training set.
    Lx, Ly : int
        Spatial / temporal size of the PEPO (qubits × time).
    TP_fn : callable, optional
        Optional map for trace-preserving checks.
    validation_data / validation_sequences : array-like, optional
        Validation split; if given, we track val loss as well.
    batch_size : int, optional
        If set, use mini-batching with vectorized_value_and_grad_batch.
    causality_key_size : int
        Number of random keys in each causality batch.
    TP_key_size : int
        Number of random keys for TP checks if TP_fn is supplied.
    refresh_causality_every : int, optional
        Re-sample causality keys every K minibatches. Default: never.
    """
    ...
    def __init__(
        ...
        executor=None,
        refresh_causality_every=None,  # optional refresh cadence
        **backend_opts,
    ):
        ...
        self.handler_val = self.handler

        self.bounds = bounds
        self.optimizer = optimizer

        # cached causality arrays + refresh policy
        self.refresh_causality_every = refresh_causality_every
        self._cached_cArrays = self.causality_fn(
            generate_random_causality_keys(
    ...
    def _maybe_update_pbar(self):
        if self._pbar is not None:
            ...
            msg = (
                f"{self.loss:+.7f} [best loss: {self.loss_best:+.7f}] "
                f"[best val: {self.val_best:+.7f}; ({np.argmin(self.val_losses)})]"
            )
    ...
    def vectorized_value_and_grad_batch(self, x, cust_kws={}):
        """The value and gradient of the loss function at vector ``x`` (mini-batch)."""

        # sample batch
        keys = np.random.choice(
            range(self._nData), self.batch_size, replace=False
        )
        tmp_data = self.training_data[keys]
        tmp_seqs = self.training_sequences[keys]

        # optionally refresh causality arrays every K evaluations
        if (self.refresh_causality_every is not None) and (
            self._n % int(self.refresh_causality_every) == 0
        ):
            self._cached_cArrays = self.causality_fn(
                generate_random_causality_keys(
                    self._Ly - 1, self._Lx, self.causality_key_size
        ...
        cust_kws["cArrays"] = self._cached_cArrays
        ...
        # validation reuses cached causality arrays too
        if self.validation_data is not None:
            val_cust = {
                "sequence_list": self.validation_sequences,
                "data": self.validation_data,
                "kappa": 1,
                "cArrays": self._cached_cArrays,
            }
            if self.TP_fn:
                val_cust["tArrays"] = TP_arrays
            self.val_loss = self.handler_val.value(arrays, val_cust)
            self.val_losses.append(self.val_loss)
            if self.val_loss < getattr(self, "val_best", float("inf")):
                self.best_val_mpo = self.get_val_tn_opt(arrays)

        self._n += 1
        self._nB = (getattr(self, "_nB", 0) + 1) % self._nBatches
        self.loss = result.item()
        self.losses.append(self.loss)
        vec_grad = self.vectorizer.pack(grads, "grad")
        self._maybe_update_pbar()
    ...
    def get_val_tn_opt(self, arrays):
        """Extract the optimized tensor network for the stored validation best.

        Parameters
        ----------
        arrays : sequence of arrays
            Arrays to inject before normalizing and tidying tags.
...
class TNOptimizer_circ:
    ...
    This is a lighter variant used for circuit-level optimisation (no
    mini-batching / causality machinery). API is compatible with quimb's
    TNOptimizer for standard use.
    """
    ...
    def __repr__(self):
        return f"<TNOptimizer_circ(d={self.d}, backend={self._autodiff_backend})>"
